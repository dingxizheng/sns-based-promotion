...
.controller('BrowseCtrl', function($scope, $injector, $ionicSideMenuDelegate, Catagory, AutoSuggest, SearchParams, $location) {
...
.controller('SearchCtrl', function($injector, $ionicHistory, $scope, $ionicTabsDelegate, $ionicSideMenuDelegate, Search, User, Promotion, SearchParams, initQuery, $ionicScrollDelegate, Session, $state, ControllerService, $localstorage, TrackService, AutoSuggest) {
    ...
    $scope.loadNext = function(index) {
        ...
        if ($scope.page[index] === 1) {
            query_params.randomize_paid = true;
        } else {
            query_params.excludes = $scope.results[index].ads.map(function(hit) { hit.result.id });
        }
        ...
        !$scope.search_done[index] && Search.query(query_params).then(function(res) {
            if ($scope.page[index] === 1) {
               if (res.data[0] && res.data[0].result.subscripted)
                    $scope.results[index].ads.push(res.data[0]);
               if (res.data[1] && res.data[1].result.subscripted)
                    $scope.results[index].ads.push(res.data[1]);
            }
            $scope.page[index] ++;
