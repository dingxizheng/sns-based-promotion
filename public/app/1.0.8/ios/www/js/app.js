...
.run(function($ionicPlatform, $cordovaDevice, $rootScope, Auth, Session, $state, $ionicLoading, Notification, $localstorage, $location, TrackService, $cordovaDialogs) {
  ...
  $ionicPlatform.ready(function() {
    ...
    // check location service every 60 seconds
...
.config(function($provide, $stateProvider, $ionicConfigProvider, $compileProvider, $urlRouterProvider, gampConfig, $httpProvider) {
  ...
  /Android/.test(navigator.userAgent) && $compileProvider.imgSrcSanitizationWhitelist(/^\s*(https?|ftp|mailto|content):/);
  ...
  .state('app.search', {
    ...
    onEnter: function(ControllerService) {
      queryStr && ControllerService.execOnEnter('search') && 
        ControllerService.execOnEnter('search')(queryStr);
      queryStr = null;
    }
  })
  ...
  $urlRouterProvider.when('/searchview/:query', function($match) {
    queryStr = $match.query;
    return '/app/search';
  });

  $urlRouterProvider.otherwise('/app/search');
  // $urlRouterProvider.otherwise('/app/loginas');
